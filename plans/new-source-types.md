...
- Synthdef: `ilex_noise` — `Select.ar(color, [WhiteNoise.ar, PinkNoise.ar, BrownNoise.ar, Crackle.ar(1.5), Dust.ar(density)])` × amp × vel × env
...
- Synthdef: `ilex_pulse` — same template as `ilex_sqr` but with `width` param exposed
...
- Synthdef: `ilex_supersaw` — center saw + 6 detuned saws spread stereo
...
- Synthdef: `ilex_sync` — master at `freq`, slave at `freq * sync_ratio`
...
- Synthdef: `ilex_ring` — carrier × modulator with depth crossfade
...
- Synthdef: `ilex_fm` — classic DX7-style 2-op FM
...
- Synthdef: `ilex_phasemod` — phase modulation (mathematically similar to FM but sounds different at edges)
...
- Synthdef: `ilex_pluck` — uses `Pluck` UGen, no sustain (naturally decaying, but still gated for note-off)
...
- Synthdef: `ilex_formant` — vowel-like timbres, formant freq controls vowel character
...
- Synthdef: `ilex_gendy` — freq_in unused (Gendy generates its own pitch range)
...
- Synthdef: `ilex_chaos` — `Select.ar(model, [HenonN, LorenzL])` scaled to audio range
...
- Synthdef: `ilex_additive` — builds harmonic series dynamically with `Mix.fill`
...
### `ilex-core/src/state/persistence/conversion.rs`
