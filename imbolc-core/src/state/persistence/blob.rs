...
mod tests {
    ...
    fn blob_round_trip_complex() {
        ...
        session.arrangement.add_placement(clip_id, saw_id, 0);
        ...
        assert_eq!(loaded_session.time_signature, (7, 8));
        assert_eq!(loaded_session.key, crate::state::music::Key::F);
        assert_eq!(loaded_session.scale, crate::state::music::Scale::Dorian);
        assert_eq!(loaded_session.tuning_a4, 442.0);
        assert!(loaded_session.snap);
        ...
        assert_eq!(
            loaded_lane.target,
            AutomationTarget::EffectParam(saw_id, 0, 0)
        );
        ...
        assert_eq!(loaded_session.midi_recording.pitch_bend_configs.len(), 1);

        // Arrangement
        assert_eq!(loaded_session.arrangement.clips.len(), 1);
        assert_eq!(loaded_session.arrangement.placements.len(), 1);
