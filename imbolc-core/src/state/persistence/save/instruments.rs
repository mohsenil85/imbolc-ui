...
pub(crate) fn save_instruments(conn: &SqlConnection, instruments: &InstrumentState) -> SqlResult<()> {
    ...
    for (pos, inst) in instruments.instruments.iter().enumerate() {
        ...
        let lfo_target_str = match inst.lfo.target {
            ...
            LfoTarget::FmIndex => "fm_index",
            LfoTarget::WavetablePosition => "wavetable_position",
            LfoTarget::FormantFreq => "formant_freq",
            LfoTarget::SyncRatio => "sync_ratio",
            LfoTarget::Pressure => "pressure",
            LfoTarget::Embouchure => "embouchure",
            LfoTarget::GrainSize => "grain_size",
            LfoTarget::GrainDensity => "grain_density",
            LfoTarget::FbFeedback => "fb_feedback",
            LfoTarget::RingModDepth => "ring_mod_depth",
            LfoTarget::ChaosParam => "chaos_param",
            LfoTarget::AdditiveRolloff => "additive_rolloff",
            LfoTarget::MembraneTension => "membrane_tension",
            LfoTarget::Decay => "decay",
            LfoTarget::Sustain => "sustain",
        };
