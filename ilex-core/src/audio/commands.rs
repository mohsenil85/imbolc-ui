...
//!
//! Phase 1: These types are defined but not yet used — AudioHandle delegates
//! directly to AudioEngine methods. In Phase 3, AudioHandle will serialize
//! commands through an MPSC channel to a dedicated audio thread.
...
///
/// Commands either carry their own data or rely on snapshots previously
/// provided via UpdateState / UpdatePianoRollData / UpdateAutomationLanes.
#[derive(Debug)]
#[allow(dead_code)]
pub enum AudioCmd {
    // ── Server lifecycle ──────────────────────────────────────────
...
///
/// In Phase 1 these are returned synchronously from AudioHandle methods.
/// In Phase 3 they'll be received via mpsc::Receiver and polled each frame.
#[derive(Debug)]
#[allow(dead_code)]
