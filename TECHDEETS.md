...
1.  Drains pending `AudioCmd`s from the main thread (non-blocking).
2.  Sleeps for a short duration (~1ms) to yield CPU.
3.  Calculates the precise elapsed `Duration` since the last tick using `std::time::Instant`.
4.  Converts this duration into **musical ticks** based on the current BPM.

### 2. Decoupled Playback
...
For high-frequency visual data (meters, waveforms) where occasional dropped frames are acceptable but blocking is not:
*   `AudioMonitor` holds `Arc<Mutex<...>>` for meters and waveforms.
*   This is the *only* shared mutex state. It is optimized for extremely short lock durations (copying a few floats).
